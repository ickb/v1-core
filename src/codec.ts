...
export type PackableOrder = PackParam<typeof UdtData> &
  PackParam<typeof PartialOrderData>;
export type UnpackedOrder = UnpackResult<typeof UdtData> &
  UnpackResult<typeof PartialOrderData>;

export const OrderData = createBytesCodec<UnpackedOrder, PackableOrder>({
  pack: (unpacked) => {
    return bytes.concat(
      UdtData.pack(unpacked),
      PartialOrderData.pack(unpacked),
    );
  },
